		...
		for (int j = -1; j < 2; j++) {
			if (IsInBounds (row + i * 2, - col - j * 2)) {
				...
				if(adjSpace.tag == "Tile")
					Debug.Log ("adjSpace collidng? " + adjSpace.GetComponent<AdjTile> ().collidingWith + " " + adjSpace.GetComponent<AdjTile> ().pos);
				if(adjSpace.tag == "Warp")
					Debug.Log ("adjSpace colliding " + adjSpace.GetComponent<Warp> ().collidingWith + " " + adjSpace.GetComponent<Warp> ().pos);


				if (CanMove ())
					moves.Add (new Vector3 (row + i * 2, col + j * 2, 0));

				//Checking regular jump space
				if (IsInBounds (row + i * 4, - col - j * 4)) {
					
					jumpSpace = Board.boardData [row / 2 + i * 2, (-col / 2) - j * 2];
					if(jumpSpace.tag == "Tile")
					Debug.Log ("This jumpSpace at pos " + jumpSpace.GetComponent<AdjTile> ().pos + " is a " + jumpSpace);
					else
						Debug.Log ("This jumpSpace at pos " + jumpSpace.GetComponent<Warp> ().pos + " is a " + jumpSpace);
					if (CanJump ()) {
