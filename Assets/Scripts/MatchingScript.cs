...
public class MatchingScript : MonoBehaviour {
	...
	void Start () 
	{
		...
		StartCoroutine(performGameMatching()); // make everything wait 5 seconds per
	}


	private IEnumerator performGameMatching()
	{
		...
		DisplayError("Checking if already in a game...",false);
		if (phase==0) {
			Debug.Log ("Checking if THIS logged in user is already in a game...(5 second wait)");
			//////////////////////////////////////////////////
			/// This used to be a seperate method... but that didn't work out well... sooo... ugh....
			/// 
		...
		// ====================================
		DisplayError("Checking if someone else is looking for a challenger...",false);
		if (phase==1) {
			Debug.Log ("Checking if someone else is waiting on a match...(5 second wait)");
			//StartCoroutine (checkIfPlayerWaiting ());
		...
		if (phase==2) {
			...
			this.gameMatch = new ParseObject(gameObjName)
			{
				...
				{"thisBoardState",  Board.getNullBoardToString()}
			};
